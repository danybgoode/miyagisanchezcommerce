...
test.describe('partners recruiting v3 · activation and serialization', () => {
  ...
  test('routes gate activation, preserve an audited resend, and use one Clerk binding writer', () => {
    ...
    expect(activationApi).toContain('completeFoundingOperatorActivation(token, user.id, user.emailAddresses)')
    expect(activationApi).toContain("result.reason === 'unavailable' ? 'unavailable' : 'invalid'")
    expect(activationApi).not.toContain('user.emailAddresses.filter')
    ...
    expect(activationPage).toContain("status === 'unavailable' ? ui.unavailable")
    ...
    expect(resend).toContain('application.activation_used_at != null')
    expect(admin).toContain('function invitationProviderLabel(value: unknown): string')
    expect(admin).toContain('function applicationActionErrorLabel(value: unknown): string')
    const decide = admin.slice(admin.indexOf('async function decide('), admin.indexOf('async function resendInvitation'))
    const resendInvitation = admin.slice(admin.indexOf('async function resendInvitation'), admin.indexOf('async function decideTransfer'))
    expect(decide).toContain('applicationActionErrorLabel(data?.error)')
    expect(resendInvitation).toContain('applicationActionErrorLabel(data?.error)')
    expect(decide).not.toContain("setMsg(data?.error")
    expect(resendInvitation).not.toContain("setMsg(data?.error")
    expect(admin).not.toContain('INVITATION_PROVIDER_LABEL[')
    expect(admin).toContain('const application = applications.find((candidate) => candidate.id === id)')
    expect(admin).toContain("application.program_track !== 'founding_operator'")
    expect(email).toContain('partnersRecruiting.email')
    expect(email).not.toContain("const subject = 'Activate your Miyagi Partners identity'")
    expect(bilingual).toContain("'partnersRecruiting'")
    expect(Object.keys(es.partnersRecruiting)).toEqual(Object.keys(en.partnersRecruiting))
    expect(en.partnersRecruiting.workspace.shopsUnavailable).toBeTruthy()
    expect(es.partnersRecruiting.workspace.shopsUnavailable).toBeTruthy()
  ...
  test('workspace branches on track without exposing MYP codes or operator administer shortcuts', () => {
    ...
    expect(en.partnersRecruiting.workspace.trackLabel).toBe('Founding Commerce Operator')
    expect(page).toContain('{!foundingOperator && promoter ? (')
    expect(page).toContain('{!foundingOperator && (')
    expect(page).toContain('<Link href="/shop/manage"')
    expect(page).toContain('const showPortfolio = portfolioEnabled && !foundingOperator')
    expect(page).toContain('{showPortfolio && <PartnerPortfolio')
    expect(page).toContain('operatorUi!.roleManager')
    expect(page).toContain('operatorUi!.operatingMarketUnavailable')
    expect(page).toContain('operatorUi!.viewShop')
    expect(page).not.toContain("title: 'Mis tiendas — Socios'")
    expect(en.partnersRecruiting.workspace.roleManager).toBe('Manager')
    expect(en.partnersRecruiting.workspace.viewShop).toBe('View shop')
  })
})
