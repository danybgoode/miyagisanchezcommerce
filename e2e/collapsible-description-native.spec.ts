...
test('S3.3 · D15 — CollapsibleDescription is server-rendered native details with a findable body', () => {
  ...
  expect(source).toContain("const wordBreak = Math.max(clipped.lastIndexOf(' '), clipped.lastIndexOf('\\n'), clipped.lastIndexOf('\\t'))")
  expect(source).toContain("const teaser = wordBreak > 0 ? clipped.slice(0, wordBreak).trimEnd() : clipped")
