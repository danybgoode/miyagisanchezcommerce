...
const PAGES = ['/mx/vende/promotor', '/mx/vende/promotor/sell-sheet']
...
test.describe('promoter landing · mobile no-overflow sweep (browser)', () => {
  ...
  test('/mx/vende/promotor shows the copy-paste prompt button at 360px', async ({ page }) => {
    await page.setViewportSize({ width: 360, height: 844 })
    const res = await page.goto('/mx/vende/promotor')
    expect(res?.ok()).toBeTruthy()
