...
test.describe('partners recruiting v3 · schema, gate and population guards', () => {
  ...
  test('the English-default US recruiting journey is dictionary-backed and exposes Spanish', () => {
    ...
    expect(en.partnersRecruiting.landing.truthBody).toContain('open marketplaces in Mexico and the United States')
    expect(en.partnersRecruiting.landing.truthBody).not.toContain('no open marketplace')
    expect(es.partnersRecruiting.landing.truthBody).toContain('marketplaces abiertos en México y Estados Unidos')
    expect(es.partnersRecruiting.landing.truthBody).not.toContain('no tiene un marketplace abierto')
