      ...
      properties: {
        configuration: {
          ...
          properties: {
            ...
            support:        { type: 'object', description: 'Support-widget (tips) config: enabled (boolean), preset_amount_cents (EXACTLY 3 integers), custom_min_cents (≥100), custom_max_cents (≤500000, min≤max, presets in range), currency (3-letter, MXN), default_visibility (public|private). ⚠️ Enabling it PROVISIONS A REAL, purchasable support product in the shop catalog — the response names its product_id. support_product_id is server-assigned and ignored if sent.' },
