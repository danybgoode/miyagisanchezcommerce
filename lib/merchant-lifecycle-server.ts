...
export async function deliverClaimedEmission(
  ...
  if (writeError) {
    // Worth shouting about: on the success path this means the row stays pending and
    // the sweep will re-send. That re-send is deduped by golden-beans on the
    // idempotency key, so the outcome is still one milestone — but a permanently
    // pending row that IS delivered would otherwise look like a stuck integration.
    console.error(
